# ============================================================
#  VOID HELPER BOT — ИСПРАВЛЕННАЯ ВЕРСИЯ v2
#  (парсинг команд !мут/!бан/!варн, список браков, /adminhelp)
# ============================================================
import asyncio
import sqlite3
import logging
...
# ---------- Парсинг целей и длительностей (ИСПРАВЛЕНО) ----------
...
async def resolve_target_from_text(chat_id, text: str):
    """Ищет цель в тексте: @username или числовой ID.
    ...
    for word in words:
        ...
        if word.isdigit():
            try:
                ...
                continue
    # Если ничего не нашли, попробуем из entities сообщения (на случай, если @упоминание замаскировано)
    # Но здесь мы работаем с уже вырезанным текстом, entities нет. Оставим как есть.
    return None, None

def extract_duration(text: str) -> Tuple[Optional[int], str]:
    """Ищет длительность в тексте (число + русские единицы времени).
       Возвращает (секунды, очищенный текст). Если не найдено, (None, исходный текст)."""
...
def parse_duration(duration_str: str) -> int:
    """Простой парсинг отдельной строки с длительностью (например, '1ч')."""
    return extract_duration(duration_str)[0] or 3600

async def parse_moderation_args(message: types.Message):
    """
    Универсальный парсер для !мут, !варн, !бан.
    Возвращает (duration, target_id, target_name).
    Если цель не найдена, target_id=None.
    """
    # Отрезаем команду
...
async def help_cmd(message: types.Message):
    await message.answer(
        ...
        "<b>💍 Браки:</b> +брак @user, +развод, +список браков\n"
        "<b>🛡 Модерация:</b> !мут, !варн, !бан, !разбан, !кик, !амнистия, !банлист\n"
...
async def del_welcome_cmd(message: types.Message):
    ...
    if not r: return await message.answer("❌ Приветствия нет")
...
async def new_member_handler(message: types.Message):
    ...
    for member in message.new_chat_members:
        ...
        try:
            if media_type == "photo":          await bot.send_photo(chat_id, file_id, caption=text, message_thread_id=tid(message))
            elif media_type == "video":        await bot.send_video(chat_id, file_id, caption=text, message_thread_id=tid(message))
            elif media_type == "animation":    await bot.send_animation(chat_id, file_id, caption=text, message_thread_id=tid(message))
        except Exception as e:
            logging.error(f"Welcome: {e}")

# ---------- Экономика и игры (без изменений) ----------
...
async def top_cmd(message: types.Message):
    ...
    for i,(uid,coins,name) in enumerate(rows):
        ...
        lines.append(f"{medal} {name or f'ID{uid}'} — {coins}")
    await message.answer("\n".join(lines))
...
async def work_cmd(message: types.Message):
    ...
    now = int(time.time())
    if now - last < 600: return await message.answer(f"⏳ Через {(600-(now-last))//60}м")
...
async def daily_cmd(message: types.Message):
    ...
    last = r[0][0] if r else 0
    if now - last < 86400: return await message.answer(f"🎁 Через {(86400-(now-last))//3600}ч")
...
async def shop_cmd(message: types.Message): await message.answer("1️⃣ x2 множитель — 500\n2️⃣ Сброс работы — 300\n\n/buy 1 или /buy 2")
...
async def buy_cmd(message: types.Message):
    ...
    if item==1:
        if coins<500: return
        add_coins(uid,-500); await message.answer("✨ x2 на 1 час!")
    elif item==2:
        if coins<300: return
        add_coins(uid,-300); db("UPDATE users SET last_work=0 WHERE id=?",(uid,)); await message.answer("⚡ Сброс!")
...
async def casino_cmd(message: types.Message):
    ...
    v = dice.dice.value
    if v==64: add_coins(uid,bet*10); txt=f"🎉 ДЖЕКПОТ!\n+{bet*10}"
    elif v>=50: add_coins(uid,bet*4); txt=f"🎰 +{bet*4}"
    elif v>=30: add_coins(uid,bet*2); txt=f"+{bet*2}"
    elif v>=15: add_coins(uid,bet); txt=f"Возврат {bet}"
    else: txt=f"Минус {bet}"
...
async def darts_cmd(message: types.Message):
    ...
    await asyncio.sleep(3)
    v = dice.dice.value
    if v==6: add_coins(uid,bet*5); txt=f"+{bet*5}"
    elif v==5: add_coins(uid,bet*3); txt=f"+{bet*3}"
    elif v==4: add_coins(uid,bet*2); txt=f"+{bet*2}"
    else: txt=f"-{bet}"
    await message.answer(txt)
...
# НОВАЯ КОМАНДА: список браков
@dp.message(F.text.lower().startswith(("+список браков", "!списокбраков", "/marriages")))
async def marriage_list_cmd(message: types.Message):
    if message.chat.type not in ("group","supergroup"): return
    rows = db("SELECT user1, user2, since FROM marriages WHERE chat_id=?", (message.chat.id,), fetch=True)
    if not rows:
        return await message.answer("💍 В этом чате нет пар.")
    lines = ["<b>💍 Пары чата:</b>"]
    for u1, u2, since in rows:
        try:
            user1 = await bot.get_chat(u1)
            name1 = user1.first_name or str(u1)
        except:
            name1 = str(u1)
        try:
            user2 = await bot.get_chat(u2)
            name2 = user2.first_name or str(u2)
        except:
            name2 = str(u2)
        lines.append(f"• {name1} ❤️ {name2}")
    await message.answer("\n".join(lines))

# ---------- Модерация (исправленный парсинг) ----------
# Мут
...
async def mute_cmd(message: types.Message):
    ...
    if duration is None:
        return await message.answer("⚠️ Укажи длительность. Пример: !мут 1ч @user")
    if not target_id:
...
async def warn_cmd(message: types.Message):
    ...
    duration, target_id, target_name = await parse_moderation_args(message)
    if duration is None:
        return await message.answer("⚠️ Укажи срок варна. Пример: !варн 1ч @user")
    if not target_id:
        return await message.answer("❌ Укажи цель (@username или ответ)!")
    if await is_admin(message.chat.id, target_id):
...
async def ban_cmd(message: types.Message):
    ...
    # Оставшийся текст используем как причину (после удаления длительности и цели)
...
async def reason_cmd(message: types.Message):
    ...
    if r: await message.answer(f"Причина: {r[0][0] if r[0][0] else 'не указана'}")
    else: await message.answer("Пользователь не забанен или причина отсутствует.")
...
async def admin_promote(message: types.Message):
    ...
    if not target_id: return
    try: await bot.promote_chat_member(message.chat.id, target_id, can_manage_chat=True, can_delete_messages=True, can_restrict_members=True)
    except: pass
...
async def admin_demote(message: types.Message):
    ...
    target_id, _ = await resolve_target_from_text(message.chat.id, message.text[6:].strip())
    if not target_id: return
    try: await bot.promote_chat_member(message.chat.id, target_id, can_manage_chat=False)
    except: pass
...
# /adminhelp (ОБНОВЛЁН, добавлен список браков)
...
async def admin_help(message: types.Message):
    await message.answer(
        ...
        "⚜️⚜️<b>💍 Список браков</b> ⚜️ показывает все пары чата\n"
        "Команды: +список браков, !списокбраков, /marriages",
        parse_mode="HTML"
    )
...
async def set_schedule(message: types.Message):
    ...
    args = message.text.split()
    if len(args) < 2:
        return await message.answer("/setautoschedule <закрытие> <открытие>\n/setautoschedule off")
    if args[1].lower() == "off":
...
async def cb_profile(call: types.CallbackQuery):
    await profile_cmd(call.message); await call.answer()
@dp.callback_query(F.data == "menu_top")
async def cb_top(call: types.CallbackQuery):
    await top_cmd(call.message); await call.answer()
@dp.callback_query(F.data == "menu_econ")
async def cb_econ(call: types.CallbackQuery):
    await call.message.edit_text("/work /daily /shop /buy"); await call.answer()
@dp.callback_query(F.data == "menu_games")
async def cb_games(call: types.CallbackQuery):
    await call.message.edit_text("/casino /darts /coinflip /rps"); await call.answer()
